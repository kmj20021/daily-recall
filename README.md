...
2. generator — Gemini API(JSON 모드)로 QAItem JSON 생성·검증(1회 재시도 + 모델 폴백)
...
- 로컬: `.env.example` 복사 → `.env`에 `GEMINI_API_KEY`, `NOTION_API_KEY`, `NOTION_DB_ID` 채우기.
  (Gemini 키는 [Google AI Studio](https://aistudio.google.com/apikey)에서 발급)
...
- `MODEL`(기본 gemini-2.5-flash, env `DR_MODEL`로 오버라이드)
- `MODEL_FALLBACK`(기본 gemini-2.5-flash-lite, env `DR_MODEL_FALLBACK`) — 기본 모델 실패 시 승계. 기본과 같게 두면 비활성
